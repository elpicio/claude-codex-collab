...
def get_session_files_with_mtime(base_path: Path, days_back: int = 7) -> list:
    ...
    for days in range(days_back):
        ...
        date_path = (
            base_path
            / str(search_date.year)
            / f"{search_date.month:02d}"
            / f"{search_date.day:02d}"
        )
...
def find_latest_token_count_record(
    base_path: Optional[Path] = None, silent: bool = False
    ...
    today_date_path = (
        base_path / str(current_time.year) / f"{current_time.month:02d}" / f"{current_time.day:02d}"
    )
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    if record.get("type") == "event_msg" and validate_token_count_record(record):
...
def format_token_usage(usage_data: Dict[str, int]) -> str:
    ...
    return (
        ...
        f"reasoning {reasoning_tokens}, subtotal {total_tokens}"
    )


def calculate_reset_time(
    rate_limit: Dict[str, Any], record_timestamp: datetime
...
def get_rate_limit_data(base_path: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if primary:
            primary_reset_time, primary_seconds_until_reset, primary_outdated = (
                calculate_reset_time(primary, record_timestamp)
            )
    ...
    try:
        ...
        if secondary:
            secondary_reset_time, secondary_seconds_until_reset, secondary_outdated = (
                calculate_reset_time(secondary, record_timestamp)
            )
            ...
            if current_time - last_refresh >= refresh_interval:
                ...
                if not data:
                    ...
                    try:
                        ...
                        header_fill = " " * (content_width - header_padding - len(header))
                        stdscr.addstr(2, 2, f"│{' ' * header_padding}{header}{header_fill}│")
                    ...
                    if "primary" in data:
                        ...
                        # Session time bar
                        reset_time_str = (
                            primary["reset_time"].astimezone().strftime("%m-%d %H:%M:%S")
                        )
                    ...
                    if "secondary" in data:
                        ...
                        # Weekly time bar
                        reset_time_str = (
                            secondary["reset_time"].astimezone().strftime("%m-%d %H:%M:%S")
                        )
                    ...
                    try:
                        ...
                        last_update_content = (
                            f" Last update: {data['current_time'].strftime('%Y-%m-%d %H:%M:%S')}"
                        )
                        ...
                        refresh_content = (
                            f" Refresh interval: {refresh_interval}s | Press 'q' to quit"
                        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parse Claude Code session token usage and rate limits"
    )
    parser.add_argument(
        "--input-folder",
        "-i",
    ...
    parser.add_argument("--live", action="store_true", help="Launch TUI live monitoring interface")
    parser.add_argument(
        "--interval",
        type=int,
        default=10,
    ...
    if not validate_token_count_record(record):
        if args.json:
            print(
                json.dumps(
                    {"error": "Found token_count event has invalid or missing required properties"}
                )
            )
        else:
    ...
    try:
        ...
        if args.json:
            ...
            if "limit_5h" in output_data:
                ...
                print(
                    f"5h limit: used {limit_5h['used_percent']}%, "
                    f"reset: {limit_5h['reset_time']}{outdated_str}"
                )
            ...
            if "limit_weekly" in output_data:
                ...
                print(
                    f"weekly limit: used {limit_weekly['used_percent']}%, "
                    f"reset: {limit_weekly['reset_time']}{outdated_str}"
                )
